...
<a href="https://github.com/iQingshan">
...
| 中文 | English |
| :--- | :--- |
| 专注 **实战攻防**：红队内网渗透、权限维持、免杀与 EDR 对抗、流量伪装与隐蔽通道；同时做蓝队视角的检测与加固验证。 | Focused on **real-world offensive security**: internal-network penetration, persistence, AV/EDR evasion, traffic disguise and covert channels — plus detection & hardening validation from the blue-team side. |
| 做 **漏洞挖掘** 与复现：从代码与协议实现里找问题，输出可复现的 PoC 与成因分析（含公开 CVE 的复现与利用链整理）。 | **Vulnerability research** and reproduction: finding flaws in code and protocol implementations, producing reproducible PoCs and root-cause analysis (including public CVE reproduction and exploit-chain write-ups). |
| 喜欢 **写工具** 解决问题：把重复的手工动作沉淀成可复用的工具与脚本，能单文件部署、开箱即用。 | I enjoy **building tools** that solve real problems — turning repetitive manual work into reusable tools that deploy as a single binary and work out of the box. |
| 做 **代码审计**：Java / PHP / Go / Python 为主，擅长从入口一路追到危险函数，关注反序列化、注入、认证绕过、越权与 SSRF 等常见面。 | **Code auditing** across Java / PHP / Go / Python: tracing from entry points down to sinks, with emphasis on deserialization, injection, auth bypass, broken access control and SSRF. |
...
- **[jsou](https://github.com/iQingshan/jsou)** — 江苏开放大学学习平台相关脚本（Python）。
- **[PageKIt_Exp](https://github.com/iQingshan/PageKIt_Exp)** — PageKit 相关利用研究。
- **保留的漏洞复现库 / Kept CVE reproductions**：[CVE-2025-49844](https://github.com/iQingshan/CVE-2025-49844)（Redis Lua 解析器 use-after-free）、[CVE-2025-32023](https://github.com/iQingshan/CVE-2025-32023)（PlaidCTF 2025 "Zerodeo"）、[CVE-2025-33073](https://github.com/iQingshan/CVE-2025-33073)（NTLM 反射 SMB 缺陷）、[AutoZerologon](https://github.com/iQingshan/AutoZerologon)、[nacos-poc](https://github.com/iQingshan/nacos-poc)。
...
| 中文 | English |
| :--- | :--- |
| 本主页展示的所有工具与代码 **仅用于获得明确授权的安全测试、攻防演练与安全研究**。请勿用于任何未授权的入侵、攻击、数据窃取或破坏行为。使用者须自行遵守所在地法律法规，并对自己的行为及后果负责。 | All tools and code shown here are intended **solely for authorized security testing, red-team exercises and research**. Do not use them for unauthorized intrusion, attack, data theft or damage. Users are responsible for complying with applicable laws and for the consequences of their actions. |
